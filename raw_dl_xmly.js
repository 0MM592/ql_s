...
//store.set('qiandao', 2);
//
//console.log(store.get('qiandao')); //=> '这是需要存储的内容'
        ...
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if (data.data.status == 0) {
                    ...
                    console.log('\n【签到成功】: '+data.data.msg)
                    console.log('\n【获得积分】: '+data.data.desc)
                    var qian = store.get('qiandao')
                    if (store.get('canshu') == 1)
                    {
                        store.set('qiandao',qian+1)
                        if (store.get('qiandao') > 7)
                        {
                            store.set('qiandao',1)
                        }
                        store.set('canshu', 2)
                    }
                    $.message +=  '\n【已签到】: '+ store.get('qiandao') + '天'
                    $.message +=  '\n【喜马拉雅用户】: '+data.context.currentUser.nickname
                    $.message +=  '\n【签到成功】: '+data.data.msg
                    $.message +=  '\n【获得积分】: '+data.data.desc
                    // var qian = store.get('qiandao')
                    // if (store.get('canshu') == 1 && store.get('qiandao') >= 7)
                    // {
                    // 	store.set('qiandao',1)
                    // }
                    // else if (store.get('canshu') == 1 && store.get('qiandao') != 7)
                    // {
                    // 	store.set('qiandao',qian+1)
                    // }
                    // else (store.get('canshu') == 1)
                    // {
                    //     store.set('canshu', 2);
                    // }
                } else {
